...
class geosumThree(MovingCameraScene):
    def construct(self):
        color1="#00FFFF"
        color2="#FF007F"
        frects=[]
        urects=[]
        shells=[]
        theFormula=MathTex(r"\frac{1}{3}","+",r"\left(\frac{1}{3}\right)^2","+",r"\left(\frac{1}{3}\right)^3","+",r"\left(\frac{1}{3}\right)^4","+",r"\left(\frac{1}{3}\right)^5","+",r"\cdots","=",r"\frac{1}{2}").scale(.85)
        ...
        for i in range(15):
            ...
            if i==0:
                self.play(Create(tempShell[0]), run_time=2)
                self.wait()
        ...
        self.play(
            self.camera.frame.animate.set_height(.33).move_to((0,0,0)),
            run_time=5
        )
        self.wait(.75)
        ...
        self.wait(2)
        finalForm=MathTex(r"\sum_{i=1}^\infty",r"\left(\frac{1}{3}\right)^i","=",r"\frac{1}{2}").scale(.85)
